@ HOW TO COMPILE AND RUN: g++ lab8prb1.s && ./a.out
@ Danielle
@ 12-05-2024 Lab 8 Problem 2: Selection sort on random array of numbers
@ Based off of sort-1.s. 
@ Notes: BUG when user input's their number. It saves the input, but then has a second
@ 		 empty line that waits for a 2nd input. BUT it doesn't do anything with the 2nd input.
.global main
...
outStr2: .asciz "%d, "
outRandArr: .asciz "\nRandom Array:\n"
outSortArr: .asciz "Sorted Array:\n"
instruct: .asciz "\nProgram searches for your number inside of the Random Array.\nProgram returns the index of your number or -1 if it's not found.\n"
inUsrNum: .asciz "Input a number: "
outNumIndx: .asciz "User Num: %d\nindex: %d\n\n"
outUsrInput: .asciz "\nUser Number: %d\n\n"
...
main: 					@ int main(){
	...
	@@ ------  Set & Print user's num ------ @@
	ldr r0, =userNum
	bl setUserNum		@ setUserNum(userNum)
	...
	@@-------- Search for input in sorted array --------@@
	ldr r0, =array
	ldr r1, =size
	ldr r1, [r1]
	ldr r2, =userNum
	ldr r2, [r2]
	bl searchArr
	mov r4, r0 		    @ searchArr() returns the index for user's num || -1	

	@@-------- Search Results --------@@
	ldr r0, =outNumIndx 
	ldr r1, =userNum	@ user num
	ldr r1, [r1]
	mov r2, r4		    @ index of user's number||-1
	bl printf			@ printf(outNumIndx)
...
@@ -------------------- Search for user's num -------------------- @@
searchArr:					@ int searchArr(array,size,userNum)
@{
	@ r4 = r0 = array address
	@ r5 = r1 = size
	@ r6 = r2 = userNum
	...
	mov r5, r1			@ size
	mov r6, r2			@ userNum
	mov r8, #0xFFFFFFFF @ numIndx=-1  @#0xFFFFFF9D  @ int stop=-99 
	mov r7, #0			@ i=0
	forILoop:			@ for(int i=0; i<size; i++){
	cmp r7,	r5			@ (i-size)==Set Flags
	bge endForLoop

	ldr r4, [r0, r7, lsl #2]	@array[i]

	cmp r4, r6			@ (array[i] - userNum)==Set Flags
	bne incrmntLoop			@ (array[i] != userNum) Z==0 FALSE

	mov r8, r7			@ r8=numIndx=i
